...
PorcentajePoblacion <- paste(format(TotalCasosOrigen / poblESP *100, digits = 4, 
    decimal.mark = ","), "%")
dfCasosMensuales <- data.frame(Mes = head(Meses, length(CasosMensuales)), Casos =  CasosMensuales)
...
En el siguiente gráfico se representan las incidencias acumuladas por cada 100.000 habitantes a lo largo de 14 días (azul) y 7 días (rojo):
...
g <- g + geom_col(aes(y=IA14), colour="blue")
g <- g + geom_col(aes(y=IA7), colour="red")
g
```

## Comparación de la evolución del número de casos entre Cantabria y Madrid

En el siguiente gráfico se compara la evolución de la enfermedad entre dos comunidades muy diferentes, Cantabria (rojo) y la Comunidad Autónoma de Madrid (azul):
...
g <- g + geom_col(aes(y=CasosMAD), colour="blue")
g <- g + geom_col(aes(y=CasosCANT), colour="red")
g
```
...
g <- ggplot(IncidenciaCantabriaMadrid, aes(Fecha)) + labs(y = "Incidencia (Casos/
                                                          100.000 hab.)")
g <- g + geom_step(aes(y=IncidMAD), colour="blue")
g <- g + geom_step(aes(y=IncidCANT), colour="red")
g
```

Por completar la información comparativa entre ambas comunidades se adjunta también la incidencia acumulada en 14 días para ambas áreas geográficas (Cantabria en rojo, Madrid en azul):

```{r Incidencias acumuladas 14 días Cantabria y Madrid}
InicioPeriodo14 <- as.Date("2020-01-01")
FinPeriodo14 <- InicioPeriodo14 + 13
...
g <- ggplot(IA14_CantabriaMadrid, aes(Fecha)) + labs(y = "Incidencia acumulada 14 
                                                     días / 100.000 hab.")
g <- g + geom_step(aes(y=IA14_MAD), colour="blue")
g <- g + geom_step(aes(y=IA14_CANT), colour="red")
g
```
